...
-- WHERE wor.ModifiedDate BETWEEN '2004-01-01' AND '2004-01-31'
...
SELECT wo.WorkOrderID AS WorkOrderID, SUM(wor.ActualCost) AS actual_cost
FROM `tc-da-1.adwentureworks_db.workorderrouting` AS wor
...
ON sales_detail.SpecialOfferID = spec_offer_product.SpecialOfferID AND sales_detail.ProductID = spec_offer_product.ProductID
...
LEFT JOIN `tc-da-1.adwentureworks_db.vendorcontact` AS vendorcontact 
ON vendor.VendorID = vendorcontact.VendorID

JOIN `tc-da-1.adwentureworks_db.vendoraddress` AS vendoraddress
